...
const Base = styled(({ ...props }) => (
  ...
  ${tw`w-screen h-16 flex items-center transition-all duration-500 before:(content-[''] absolute -top-full right-full w-full h-16 bg-brand-surface opacity-95 brightness-95 transition-all duration-500)`}
  ${({ sticky }) => sticky && tw`fixed before:(top-0 right-0) z-40`}
`;
